...
class RectangleSplittingManager(object):
    ...
    def __start_worker(self, nof_worker, port, authkey, nof_cpu):
        ...
        for i in xrange(nof_worker):
            ...
            comm = command%(int(nof_cpu),  error_log, output_log, input_log)
            ...
            id = subprocess.check_output("%s %s "%(submit_call, comm), shell=True).strip()
            print id
            #print "%s %s "%(submit_call, comm) 
    ...
    def solve(self, init_recs=None):
        ...
        if init_recs:
            ...
            for i in xrange(len(init_recs)-1):
                ...
                self.task_q.put_nowait((0, 1, rec_b, warm, rec,[zi.__hash__(), zj.__hash__()]))
            ...
            while task_count:
                ...
                hashs[pos] = sol.__hash__()
            rec_b = 0.5*(b[0][1]+b[1][1])
            task_count = 1
            self.task_q.put_nowait((0, 1, rec_b, warm, b, hashs))
            print "Initial hashes ",hashs
        while task_count:

            pos, sol, warm, origin_rect, hashs = self.done_q.get()
            print 
            ...
            if pos == "error":
                print "!!!!!!!!!!!!!!!!!!"
                print origin_rect, "caused an error"
                print "!!!!!!!!!!!!!!!!!!"
                task_count -= 1
                continue
            ...
            if pos:
                print "lexmin2", pos, " testing ",sol, " vs ", origin_rect[0],hashs[0], "".join(sol.__hash__()[i] for i in xrange(len(hashs[0])) if hashs[0][i] != sol.__hash__()[i])
                if sol.__hash__() != hashs[0]:
                    rec = (origin_rect[0], sol.objs)
                    has = [ hashs[0],sol.__hash__()]
                    self.solutions.append(sol)
                    rec_b = 0.5*(rec[0][1]+rec[1][1])
                    self.task_q.put((0, 1, rec_b, warm, rec, has))
                    task_count += 1
                    
                else:
                     print "rejected ", sol, " by ", origin_rect[0]
                     print
            #lexmin1
            else:
                print "lexmin1 ",pos, " testing ",sol, " vs ", origin_rect[1], hashs[1], "".join(sol.__hash__()[i] for i in xrange(len(hashs[1])) if hashs[1][i] != sol.__hash__()[i])
                rec_t = sol.objs[0]-BiobjectiveSolver.EPS
                self.task_q.put((1, 0, rec_t, warm, origin_rect, hashs))
                task_count += 1
                if sol.__hash__() != hashs[1]:
                    rec = (sol.objs, origin_rect[1])
                    has = [sol.__hash__(), hashs[1]]
                    rec_b = 0.5*(rec[0][1]+rec[1][1])
                    self.solutions.append(sol)
                    self.task_q.put((0, 1, rec_b, warm, rec, has))
                    task_count += 1
                   #encountered[sol.__hash__()] += 1
                else:
                    print "rejected ",sol," by ", origin_rect[1]
                    print
            task_count -= 1
...
if __name__ == "__main__":
    ...
    manager = RectangleSplittingManager(args.input[0],args.input[1],["x","y"], 4,nof_cpu=4, port=6881)
    start = time.time()
    sols = manager.solve()
    end=time.time()
